...
class RSA_PKCS_1(RSA):
    ...
    def enc_PKCS_1(self, d, ps=None):
        ...
        ps_len = max(self.k - 3 - len(d), 8)
        if ps is None:
            ps = self.pad(ps_len)

        eb = b"\x00" + self.bt.to_bytes(1, 'big') + ps + b"\x00" + d  # Encryption Block
    ...
    def parse(self, eb):
        ...
        """
        if eb[0] != 0:
            raise Exception("First byte of encryption block should be 00")

        if eb[1] != 0 and eb[1] != 1 and eb[1] != 2:
            raise Exception("BT is from unknown type")

        ps_len = 0

        if self.bt != eb[1]:
            raise Exception("Block type not match")
        ...
        if eb[1] == 0:
            ...
            while eb[ps_len + 2] == 0xff:
                ps_len += 1
            if eb[ps_len + 2] != 0:
                raise Exception("Invalid padding of type 01")
        else:
            while eb[ps_len + 2] != 0:
                ps_len += 1
