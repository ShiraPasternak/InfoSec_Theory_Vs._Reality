        ...
        for r in range(max(start_round, 1), end_round + 1):
            ...
            for i in range(NumTables):
                for l in range(int(BlockSize / NumTables)):
                    accessed_list[j][i].append((stateround[(l * 4) + i] & 0xF0))
        ...
        for l in range(int(BlockSize / NumTables)):
            ...
            for j in range(num_traces):
                for nibble in unaccessed_list[j][i]:
                    tindex = (nibble ^ plaintexts[j][(i + NumTables * l)])
                    guess_list[tindex >> 4] = False;
...
def calc_ttables(l, plaintexts, partial_k0_high):
    ...
    :return: 3-dimensional array T-result, where T_result[j][nibble][i] holds the output of Table i for the j'th
        plaintext and bottom k0 nibble
    """
    num_traces = len(plaintexts)
    T_result = np.empty((num_traces, 2 ** 4, NumTables), dtype=np.uint32)

    plaintext_indexs = [[0, 5, 10, 15], [4, 9, 14, 3], [8, 13, 2, 7], [12, 1, 6, 11]]
    for j in range(num_traces):
        for nib in range(16):
            exec('; '.join([f"T_result[j][nib][{i}] = AESr.T{i + 1}[plaintexts[j][plaintext_indexs[l][{i}]] ^ partial_k0_high[{i}] ^ nib]" for i in range(4)]))
...
def find_unviable_candidates(j, T_result, k0_low, viable):
    ...
    for i in range(NumTables):
        for k in range(16):
            if not viable[k][i]:
                t_for_K0 = int(T_result[j][k0_0][0] ^ T_result[j][k0_1][1] ^ T_result[j][k0_2][2] ^ T_result[j][k0_3][3])
                unviable.append((((t_for_K0.to_bytes(4, 'big'))[i] >> 4) ^ k, i))
...
def guess_k1_ttable(plaintexts, unaccessed_list, k0, verbose=False):
    ...
    r1 = [aes.encrypt_r(plaintexts[i], 2, 0, False) for i in range(num_traces)]
...
def recover_full_key(key_len, k0_list, k1_list):
    ...
    """
    full_key_list = [(k0 + k1)[:int(key_len / 8)] for k0 in k0_list for k1 in k1_list]
