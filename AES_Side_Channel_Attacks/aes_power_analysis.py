...
def guess_key_hd(plaintexts, hamming_distances, r):
    """
    Guesses a round key based on hamming distances leaned from power analysis
    :param plaintexts: list of plaintexts
    ...
    for i in range(BlockSize):
        ...
        for guess in range(2 ** 8):
            for j in range(num_traces):
                if hamming_weight(AESr.S[plaintexts[j][i] ^ guess] ^ plaintexts[j][i] ^ guess) != hamming_distances[j][r][i]:
